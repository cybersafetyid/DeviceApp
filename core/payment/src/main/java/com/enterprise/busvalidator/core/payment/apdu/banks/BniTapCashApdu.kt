...
/**
 * BNI TapCash APDU Handler.
 * Supports AID D360000002, readcardinfo, auto completion, deduct, and TransCode generation.
 */
@Singleton
class BniTapCashApdu @Inject constructor(
    private val logger: EncryptedLogger
) : BankApduHandler {

    override val bankIssuer: BankIssuer = BankIssuer.BNI_TAPCASH

    // BNI TapCash AID: D360000002
    private val selectAidCommand = byteArrayOf(
        0x00.toByte(), 0xA4.toByte(), 0x04.toByte(), 0x00.toByte(),
        0x05.toByte(), 0xD3.toByte(), 0x60.toByte(), 0x00.toByte(), 0x00.toByte(), 0x02.toByte()
    )
    ...
    override fun readCardInfo(
        ...
        logger.log("BniTapCashAPDU", "Reading BNI TapCash Card Info for UID: $cardUid")
        ...
        var balance = 75000L
        var cardNumber = "7546-0192-${cardUid.takeLast(4).padStart(4, '0')}-4412"
        var uncompletedState = UncompletedTxState.CLOSED
        val lastTxCounter = 34
        ...
        val lastTransCode = TransCodeGenerator.generateTransCode(
            bankIssuer = BankIssuer.BNI_TAPCASH,
            cardUid = cardUid,
    ...
    override fun processMandiriGracePeriod(
        ...
        return MandiriGracePeriodResult(false, standardFare, standardFare, 0L, "Not applicable for BNI TapCash")
    }
    ...
    override fun processAutoCompletion(
        ...
        logger.log("BniTapCashAPDU", "EXECUTING BNI TAPCASH AUTO COMPLETION APDU for Card: ${cardInfo.cardNumberFormatted}")
        selectApplication(transmitCardApdu)
    ...
    override fun deduct(
        ...
        logger.log("BniTapCashAPDU", "EXECUTING BNI TAPCASH DEBIT APDU: Card=${cardInfo.cardNumberFormatted}, Amount=$amountToDeduct")
        selectApplication(transmitCardApdu)

        val samSignature = if (transmitSamApdu != null) {
            val samReq = byteArrayOf(0x80.toByte(), 0x1A.toByte(), 0x03.toByte(), 0x00.toByte(), 0x08.toByte()) + amountToDeduct.toByteArrayBigEndian(8)
            val samResp = transmitSamApdu(samReq)
            samResp.toHexString()
        } else {
            "SAM-BNI-MAC-${System.currentTimeMillis().toString().takeLast(8)}"
        }

        val debitCmd = byteArrayOf(
            0x80.toByte(), 0xDA.toByte(), 0x00.toByte(), 0x00.toByte(), 0x04.toByte()
        ...
        if (!isSuccess) {
            return ApduDeductResult(
                ...
                errorMessage = "BNI TapCash Debit APDU Rejected"
            )
        }

        val nextTxCounter = 36
        ...
        logger.log("BniTapCashAPDU", "BNI TapCash Debit APDU SUCCESS: TransCode=$transCode, FinalBal=$finalBalance")
