...
/**
 * Bank Nobu E-Money APDU Handler.
 * Supports AID D360000004, readcardinfo, auto completion, deduct, and TransCode generation.
 */
@Singleton
class BankNobuApdu @Inject constructor(
    private val logger: EncryptedLogger
) : BankApduHandler {

    override val bankIssuer: BankIssuer = BankIssuer.NOBU_EMONEY

    // Bank Nobu AID: D360000004
    private val selectAidCommand = byteArrayOf(
        0x00.toByte(), 0xA4.toByte(), 0x04.toByte(), 0x00.toByte(),
        0x05.toByte(), 0xD3.toByte(), 0x60.toByte(), 0x00.toByte(), 0x00.toByte(), 0x04.toByte()
    )
    ...
    override fun readCardInfo(
        ...
        logger.log("BankNobuAPDU", "Reading Bank Nobu E-Money Card Info for UID: $cardUid")
        ...
        var balance = 55000L
        var cardNumber = "6037-9912-${cardUid.takeLast(4).padStart(4, '0')}-0504"
        ...
        val lastTransCode = TransCodeGenerator.generateTransCode(
            bankIssuer = BankIssuer.NOBU_EMONEY,
            cardUid = cardUid,
    ...
    override fun processMandiriGracePeriod(
        ...
        return MandiriGracePeriodResult(false, standardFare, standardFare, 0L, "Not applicable for Bank Nobu E-Money")
    }
    ...
    override fun processAutoCompletion(
        ...
        logger.log("BankNobuAPDU", "EXECUTING NOBU AUTO COMPLETION APDU for Card: ${cardInfo.cardNumberFormatted}")
        selectApplication(transmitCardApdu)
    ...
    override fun deduct(
        ...
        logger.log("BankNobuAPDU", "EXECUTING NOBU DEBIT APDU: Card=${cardInfo.cardNumberFormatted}, Amount=$amountToDeduct")
        selectApplication(transmitCardApdu)

        val samSignature = if (transmitSamApdu != null) {
            val samReq = byteArrayOf(0x80.toByte(), 0x1A.toByte(), 0x05.toByte(), 0x00.toByte(), 0x08.toByte()) + amountToDeduct.toByteArrayBigEndian(8)
            val samResp = transmitSamApdu(samReq)
            samResp.toHexString()
        } else {
            "SAM-NOBU-MAC-${System.currentTimeMillis().toString().takeLast(8)}"
        }

        val debitCmd = byteArrayOf(
            0x80.toByte(), 0xD5.toByte(), 0x00.toByte(), 0x00.toByte(), 0x04.toByte()
        ...
        if (!isSuccess) {
            return ApduDeductResult(
                ...
                errorMessage = "Bank Nobu E-Money Debit APDU Rejected"
            )
        }

        val nextTxCounter = 11
        ...
        logger.log("BankNobuAPDU", "Nobu Debit APDU SUCCESS: TransCode=$transCode, FinalBal=$finalBalance")
